...
server.post("/results/", function (req, res, next) {
  const clientSelection = req.body.clientSelection;
  const { computerSelection, result } = computeResult(clientSelection);

  res.status(200).json({ computerSelection, result });
});

function computeResult(clientSelection) {
  return "this";
}
